...
#define PROGFILE "../binaries/isa/stha.bin"
#define KB 1024
#define MEMSZ (250 * KB)
...
static uint16_t __testasi_mval;

static int __testasi_write16(struct dev *dev, addr_t addr, uint16_t val)
{
	(void)dev;
	if(addr == 0xdeadbee0)
		__testasi_mval = be16toh(val);
