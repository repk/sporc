...
#define PROGFILE "../binaries/isa/ldda.bin"
#define KB 1024
#define MEMSZ (250 * KB)
...
static int __testasi_read32(struct dev *dev, addr_t addr, uint32_t *val)
{
	(void)dev;
	if(addr == 0xdeadbee0)
		*val = htobe32((uint32_t)(-12));
	else if(addr == 0xdeadbee4)
		*val = htobe32((uint32_t)(-0x12));
	else
		*val = 0;
