...
struct memory_ops {
	 ...
	 */
	int (*map)(struct memory *m, addr_t addr, off_t off, size_t sz,
			perm_t perm);
	/**
	 * Unmap a mapp'ed memory
	 */
	int (*unmap)(struct memory *m, addr_t addr, size_t sz);
	/**
	 * Read one byte
	 */
	int (*read8)(struct memory *m, addr_t addr, uint8_t *val);
	/**
	 * Read one halfword
	 */
	int (*read16)(struct memory *m, addr_t addr, uint16_t *val);
	/**
	 * Read one word
	 */
	int (*read32)(struct memory *m, addr_t addr, uint32_t *val);
	/**
	 * Write one byte
	 */
	int (*write8)(struct memory *m, addr_t addr, uint8_t val);
	/**
	 * Write one halfword
	 */
	int (*write16)(struct memory *m, addr_t addr, uint16_t val);
	/**
	 * Write one word
	 */
	int (*write32)(struct memory *m, addr_t addr, uint32_t val);
	/**
	 * Fetch one byte for exec
	 */
	int (*fetch_isn8)(struct memory *m, addr_t addr, uint8_t *val);
	/**
	 * Fetch one halfword for exec
	 */
	int (*fetch_isn16)(struct memory *m, addr_t addr, uint16_t *val);
	/**
	 * Fetch one word for exec
	 */
	int (*fetch_isn32)(struct memory *m, addr_t addr, uint32_t *val);
};
